...
(function($){
    ...
    $.fn.smartPopover = function(html, opt){
      ...
      　　  $(document).off("mousedown", destroy);
            $(".js-smartpopover").fadeOut(100, function(){$(this).remove()});
        };
        ...
        var root = $("<div/>").addClass("js-smartpopover")
                .css({position:"absolute", left:0, top:0, overflow:"visible", "z-index":zindex})
                .appendTo("body");
        var popdom = $(html).css({position:"absolute", opacity:0}).appendTo(root);
        var pop = {w:popdom.width(), h:popdom.height()};
        popdom.hide();
        ...
        var popwin = $("<div/>")
                ...
                .appendTo(root);
        popdom.css({left:margin.w, top:margin.h, opacity:1}).appendTo(popwin).show();

        pop.w = pop.w + margin.w*2;
        pop.h = pop.h + margin.h*2;

        var poporg = {w:pop.w, h:pop.h};
        ...
        var onresize = function(){
            of = $(target).offset();
            var t = {x: of.left,y: of.top, w: $(target).width(), h: $(target).height()};
            var win = {x:$(window).scrollLeft(),y:$(window).scrollTop(), w: $(window).width(), h: $(window).height()};
            ...
            popwin.css({width:pop.w, height: pop.h});

            var r = {
                left : { p: points.left.x - win.x - pop.w - arrow },
                right : { p: win.x+win.w - points.right.x - pop.w - arrow}, 
                top : { p: points.top.y - win.y - pop.h - arrow }, 
                bottom : { p: win.y+win.h - points.bottom.y - pop.h - arrow }
            }; 
            ...
            popwin.css({left:placed.x, top:placed.y});
